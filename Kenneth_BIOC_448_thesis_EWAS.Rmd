---
title: "Kenneth_BIOC_448_Thesis"
author: "Kenneth Ofori-Amanfo"
date: "`r Sys.Date()`"
...
## Preparing the Starting Files for Analysis

### Import external files
```{r}
clinical_table <- fread("UBC_ELim_Baseline_CoPv7.csv")

epigenetic_table <- fread("CLSA.Preprocessed_Betas_Transposed_Apr2021.csv")

## The CSd
pccf_file <- read.delim("PCCF_FCCP_V2409_2021.txt",
                        header = FALSE)
```

### Obtain 3 year average of CANUE's multi-pollutant score
```{r}
mp_2012 <- fread("aqaix_ava_12.csv")
mp_2013 <- fread("aqaix_ava_13.csv")
mp_2014 <- fread("aqaix_ava_14.csv")

## merge the yearly average tables into a single one by their postal codes
canue_csd_data <- left_join(mp_2012, mp_2013, 
                            by = c("postalcode12" = "postalcode13")) %>%
  left_join(., mp_2014, by=c("postalcode12" = "postalcode14")) %>%
  ## turn -9999 inputs to NA values
  mutate(aqaix12_01 = replace(aqaix12_01, which(aqaix12_01 < 0), NA)) %>%
  mutate(aqaix13_01 = replace(aqaix13_01, which(aqaix13_01 < 0), NA)) %>%
  mutate(aqaix14_01 = replace(aqaix14_01, which(aqaix14_01 < 0), NA)) %>%
  mutate(avg_mp = rowMeans(cbind(aqaix12_01, aqaix13_01, aqaix14_01), 
                           na.rm = TRUE)) %>%
  mutate(avg_mp = replace(avg_mp, which (is.nan(avg_mp)), NA))
```

### Get the mode multi-pollutant score for each CSD
```{r}
##-----------------------------------------------------------------------------
## use the PCCF file get get the Mode of postal codes in CSD

## From the pccf file, everything is in a single column. Must separate based
## on character position
# create a column for postal_code
pccf_file$postal_code <- substr(pccf_file$V1,1,6)
pccf_file$csd_id <- as.integer(substr(pccf_file$V1,16,22))

## Add the CSD to each row in to the canue data

canue_csd_data <- left_join(canue_csd_data, pccf_file, 
                            by=c("postalcode12" = "postal_code"))

## For each CSD we are going to take the Mode
...
canue_csd_data <- canue_csd_data %>% group_by(csd_id) %>%
  summarise(
    getmode(avg_mp))
```

### Shorten clinical data to relevant information for analysis and add multi-pollutant score
```{r}
...
clinical_data_mp <- clinical_table[,c("ADM_EPIGEN2_COM", "SEX_ASK_COM",
                                      ...
                                      "pm25_exp_com")] %>%
  left_join(. , canue_csd_data, by=c("MSDYY_CSD_COM" = "csd_id")) 
colnames(clinical_data_mp)[20] <- "multipollutant_exp"
```

## Summary of Cohort Stats
Characterization | Summary
-----------------|---------------
N                |1445
Female (%)       |51
Age (years)      |62.99 (± 10.30)
BMI(%)           |
  Underweight    |1
  Normal         |27
  Overweight     |40
...
Income (%)
  < $20,000      |7
  From 20,000  50,000 |26
  From 50,000 to 100,000|32
  From 100,000 to 150,000|18
  150,000 or more|17
PM2.5 exposure (ug/m^3)|6.59 (± 3.81)
NO2 (ug/m^3)|17.42 (± 11.36)
O3(ug/m^3)|46.90 (± 17.63)
SO2 (ug/m^3)|2.43 (± 3.73)
Multi-pollutant score| 25.19 (± 9.30)

```{r}
## Reduce Clinical data to have only information on those with epigenetic data
participant_data <- clinical_data_mp
participant_data <- participant_data[participant_data$ADM_EPIGEN2_COM %in% as.integer(epigenetic_table[[1]]),] 

## ----------------------------------------------------------------------------------
### Age of participants
mean(participant_data$AGE_NMBR_COM)
sd(participant_data$AGE_NMBR_COM)

#                           mean: 62.99 years of age       sd: 10.30

## ----------------------------------------------------------------------------------
### Sex of participants
percent_male <- participant_data[!is.na(participant_data$SEX_ASK_COM),] %>%
  summarise(proportion = sum(SEX_ASK_COM == "M") / n())
...
## ----------------------------------------------------------------------------------
### High School Diploma?
has_hs_diploma <- participant_data[!is.na(participant_data$ED_HSGR_COM),] %>%
  .[.$ED_HSGR_COM <= 2,] %>%
    summarise(proportion = sum(ED_HSGR_COM == 1) / n())

## 95% have graduated high school

## ----------------------------------------------------------------------------------
### Alcohol Frequency: Total participants without NA value: 1442
## Frequent alcohol consumption (2-3 times a week or more): 45% participants
nrow(participant_data[participant_data$ALC_FREQ_COM <= 3]) / 
  nrow(participant_data[!is.na(participant_data$ALC_FREQ_COM)])
## Occasional alcohol consumption (once a week to <1 a month): 42% participants
nrow(participant_data[participant_data$ALC_FREQ_COM <= 7 &
                                       participant_data$ALC_FREQ_COM >= 4])/
  nrow(participant_data[!is.na(participant_data$ALC_FREQ_COM)])
## Zero alcohol consumption (never) : 13% participants
nrow(participant_data[participant_data$ALC_FREQ_COM == 96])/
  nrow(participant_data[!is.na(participant_data$ALC_FREQ_COM)])



## -------------------------------------------------------------------------
### Average BMI classifications
## from 1-6 respresenting underweight to obese respectively
## pregnant woman (96) and other (99) were excluded

## Underweight (1): 1%
nrow(participant_data[participant_data$HWT_DISW_COM == 1,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM) |
                          participant_data$HWT_DISW_COM < 96,])
## Normal weight (2) 27%
nrow(participant_data[participant_data$HWT_DISW_COM == 2,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM)|
                          participant_data$HWT_DISW_COM < 96,])
## Overweight (3) 39%
nrow(participant_data[participant_data$HWT_DISW_COM == 3,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM)|
                          participant_data$HWT_DISW_COM < 96,])
## Obese - Class I (4) 22%
nrow(participant_data[participant_data$HWT_DISW_COM == 4,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM)|
                          participant_data$HWT_DISW_COM < 96,])
## Obese - Class II (5) 7% %
nrow(participant_data[participant_data$HWT_DISW_COM == 5,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM)|
                          participant_data$HWT_DISW_COM < 96,])
## Obese - Class III  4%
nrow(participant_data[participant_data$HWT_DISW_COM == 6,]) /
  nrow(participant_data[!is.na(participant_data$HWT_DISW_COM)|
                          participant_data$HWT_DISW_COM < 96,])
...
## Daily smokers: 13%
nrow(participant_data[participant_data$SMK_CURRCG_COM == 1,]) /
  nrow(participant_data[!is.na(participant_data$SMK_CURRCG_COM),])
## Occasional Smokers: 3%
nrow(participant_data[participant_data$SMK_CURRCG_COM == 2,]) /
  nrow(participant_data[!is.na(participant_data$SMK_CURRCG_COM),])
##Done Smoke (have not in the past 30 days): 84
nrow(participant_data[participant_data$SMK_CURRCG_COM == 3,]) /
  nrow(participant_data[!is.na(participant_data$SMK_CURRCG_COM),])

## ------------------------------------------------------------------------
# ### Ethnicity

## RATHER THAN DISPLAYING PERCENTAGES, WE WILL PUT HOW MANY INDVIDUALS IDENTIFY
## WITH THE ETHNIC GROUP. THEN WE GENERALIZE BASED ON GEOGRAPHIC LOCATION
## BECAUSE WE DO NOT HAVE ENOUGH REPRESENTATION 
# ## change data to long data to include ethnicity from both sides
ethn_df <- pivot_longer(participant_data,cols = c("SDC_ETHN_OTSP01_COM","SDC_ETHN_OTSP02_COM"),
                        names_to="SDC_col",
                        values_to="ethnicity") # %>%

## Ethnic Origins Related to North America (First nations): 9
ethn_df[!is.na(ethn_df$ethnicity),] %>%
  .[.$ethnicity == 21,] %>%
  nrow()
## Ethnic Origins Related to Europe: 161
ethn_df[!is.na(ethn_df$ethnicity),] %>%
  .[.$ethnicity >= 22 & .$ethnicity <= 44,] %>%
  nrow()
## Ethnic Origins Related to South America: 11
ethn_df[!is.na(ethn_df$ethnicity),] %>%
  .[.$ethnicity >= 45 & .$ethnicity <= 53,] %>%
  nrow()
## Ethnic Origins Related to Africa: 6
ethn_df[!is.na(ethn_df$ethnicity),] %>%
  .[.$ethnicity >= 54 & .$ethnicity <= 61,] %>%
  nrow()
## Ethnic Origins Related to Asia: 23
ethn_df[!is.na(ethn_df$ethnicity),] %>%
  .[.$ethnicity >= 62 & .$ethnicity <= 74,] %>%
  nrow()

##----------------------------------------------------------------- ## Income 

## 1: less than 20 000
nrow(participant_data[participant_data$INC_TOT_COM == 1,]) /
  (1445-78)
## 2: 20 000 < x > 50 000
nrow(participant_data[participant_data$INC_TOT_COM == 2,]) /
  (1445-78)
## 3: 50 000 < x > 100 000
nrow(participant_data[participant_data$INC_TOT_COM == 3,]) /
  (1445-78)
## 4: 100 000 < x> 150 000
nrow(participant_data[participant_data$INC_TOT_COM == 4,]) /
  (1445-78)
## 5: 150 000 or more
nrow(participant_data[participant_data$INC_TOT_COM == 5,]) /
  (1445-78)
## 7,8,9: missing, refused, don't know
nrow(participant_data[participant_data$INC_TOT_COM > 5,]) /
  nrow(participant_data[!is.na(participant_data$INC_TOT_COM),])

## -------------------------------------------------------------------------
### Pollutant averages
# pm25_exp_com       6.59 microgram/m^3         sd 3.81
mean(participant_data$pm25_exp_com[participant_data$pm25_exp_com >= 0],
     na.rm = TRUE)
sd(participant_data$pm25_exp_com[participant_data$pm25_exp_com >= 0],
   na.rm = TRUE)

# no2_exp_com         9.26 ppb                  sd 6.04
# ppb*46.0055/24.45   17.37 microgram/m^3       sd 11.36
mean(participant_data$no2_exp_com[participant_data$no2_exp_com >= 0],
     na.rm = TRUE)
sd(participant_data$no2_exp_com[participant_data$no2_exp_com >= 0],
   na.rm = TRUE)

# o3_exp_com        23.89 ppb                   sd 8.98
# (ppb*48/24.45)    46.90 microgram/m^3         sd 17.63
mean(participant_data$o3_exp_com[participant_data$o3_exp_com >= 0],
     na.rm = TRUE)
sd(participant_data$o3_exp_com[participant_data$o3_exp_com >= 0],
   na.rm = TRUE)

# so2_exp_com        0.93 ppb                   sd 1.433
# (ppb*64.066/24.45) 2.43 microgram/m^3         sd 3.73
mean(participant_data$so2_exp_com[participant_data$so2_exp_com >= 0],
     na.rm = TRUE)
sd(participant_data$so2_exp_com[participant_data$so2_exp_com >= 0],
   na.rm = TRUE)

## Multipollutant exposure
mean(participant_data$multipollutant_exp[participant_data$multipollutant_exp >= 0],
     na.rm = TRUE)
sd(participant_data$multipollutant_exp[participant_data$multipollutant_exp >= 0],
   na.rm = TRUE)
...
## Designed functions
  ### Function for the generalized linear model
```{r}
...
pollutant_glm <- function(pollutant,s1_name,s2_name,s3_name,s4_name){
  write.csv(first_subset <- subset_mp_glm(pollutant,2,200000), s1_name)
  write.csv(second_subset <- subset_mp_glm(pollutant, 200001,400000), s2_name)
  write.csv(third_subset <- subset_mp_glm(pollutant, 400001,600000), s3_name)
  write.csv(fourth_subset <- subset_mp_glm(pollutant, 600001, 783137), s4_name)}
```

  ### Functions for annotated CpGs
```{r}
##---------------------------------------------------------------------------
...
```
  ...
  ### Find CpGs with a trend towards association (FDR < 0.5)
```{r}
##----------------------------------------------------------------------------
...
significant_values <- function(df){
 ...
 #test1 <-subset_mp_glm("aqaix12_01", 405601 ,406601)
```
...
pollutant_glm("multipollutant_exp","mp3_first_subset.csv","mp3_second_subset.csv",
              "mp3_third_subset.csv","mp3_fourth_subset.csv")
...
```
  ...
  ### Read  and annotate the glm's CSV output
    #### Also find significant values
```{r}
## Multipollutant

mp_first_subset <- fread("mp3_first_subset.csv")[,-1]
mp_second_subset <- fread("mp3_second_subset.csv")[,-1]
mp_third_subset <- fread("mp3_third_subset.csv")[,-1]
mp_fourth_subset <- fread("mp3_fourth_subset.csv")[,-1]
...
exp_first_subset <- fread("pm25_exp_com_first_subset")[,-1]
exp_second_subset <- fread("pm25_exp_com_second_subset")[,-1]
exp_third_subset <- fread("pm25_exp_com_third_subset")[,-1]
exp_fourth_subset <- fread("pm25_exp_com_fourth_subset")[,-1]
...
no2_first_subset <- fread("no2_exp_com_first_subset")[,-1]
no2_second_subset <- fread("no2_exp_com_second_subset")[,-1]
no2_third_subset <- fread("no2_exp_com_third_subset")[,-1]
no2_fourth_subset <- fread("no2_exp_com_fourth_subset")[,-1]
...
o3_first_subset <- fread("o3_com_first_subset")[,-1]
o3_second_subset <- fread("o3_com_second_subset")[,-1]
o3_third_subset <- fread("o3_com_third_subset")[,-1]
o3_fourth_subset <- fread("o3_com_fourth_subset")[,-1]
...
so2_first_subset <- fread("so2_exp_com_first_subset")[,-1]
so2_second_subset <- fread("so2_exp_com_second_subset")[,-1]
so2_third_subset <- fread("so2_exp_com_third_subset")[,-1]
so2_fourth_subset <- fread("so2_exp_com_fourth_subset")[,-1]
...
## **Aim 1:** Individual CpGs associated with air pollution
  ### Top CpG sites of GLM output
```{r}
## Multi-pollutant
...
write_csv(mp_top25, "mp3_top25.csv")
...
## **Aim 2:** Genomic Regions associated air pollution
  ### CGI-related regions - forest plot
```{r}
## Multi-pollutant
...
```
  ### Gene structure related regions - forest plot
```{r}
## Multi-pollutant
...
forest_plot_genes_so2 <- ggplot(data=df_region_relation_so2, aes(x=label, y= mean,
  ...
  theme(legend.position = "none")
```
  ### Islands and shores in Gene structure related regions - forest plot
```{r}
## Multi-pollutant
...
## **Aim 3:** Enrichment Analysis
  ### GO and KEGG gene sets
```{r}
## Multi-pollutant
...
## Points to help with the discussion section
```{r}

pollomic <- fread("Methylation_dynamic_table.csv")

pollomic_transcription <- fread("Transcript_dynamic_table.csv")
...
```
  ...
  ### Gene structure-related forest plots
```{r}
## Arrange forest plots into a single figure
...
```
  ...
  ### CGIs in gene structured-related regions forest plots
```{r}
